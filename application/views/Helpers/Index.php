<?php
    ...
    if($HasActions && !isset($Actions['EDIT']))$Actions['EDIT'] = ['Title'=>'<i class="fa fa-pencil"> Edit</i>','Link'=>AURI('Edit/{Row.ID}'),'Attributes'=>['']];
    if($HasActions && !isset($Actions['DELETE']))$Actions['DELETE'] = ['Title'=>'<i class="fa fa-trash"> Delete</i>','Link'=>AURI('Action/Delete/{Row.ID}'),'Attributes'=>[]];

    //Defining Multiple Actions
    if($HasActions && !isset($MultiActions['ADD']))$MultiActions['ADD'] = '{(_A(\'<i class="fa fa-plus-circle"></i>\',AURI(\'Add\'),[\'class\'=>\'btn btn-success\']))}';
    ...
    //Editable Action
    if($Editable){
        $Bundels->Snippet('PreScripts',sprintf('<script type="text/javascript">var SaveEditable = \'%s\';var CFN = \'%s\';var CFH = \'%s\';</script>',AURI(sprintf('SaveEditable/%s',$EditModel)),_CSRF(),_CSRF_Hash()));
        $Headers = array();
        $DetailHeaders = array();
...
<?php endif;?>
<form action="<?php echo AURI('Action/General')?>" method="post">
    <div class="card">
                    ...
                    <table data-type="DataTable" class="table" style="min-width: 100%;">
                        ...
                        <?php
                            if($List && $List->Rows > 0 && !empty($Headers)):
                                ...
                                if($Multi)$TotalCells++;
                                echo sprintf('<tbody><tr><td colspan="%d"><div class="alert alert-warning">No record was found.%s</div></td></tr></tbody>',$TotalCells,GS('F') ? sprintf(' Filter map apply try to clear the filters. %s',_A('Clear Filters',AURI($this->Method),['class'=>'link'])) : '');
                            endif;
                        ?>
                    </table>
                </div>
